<template>
    <form class="w-full max-w-sm mx-auto mt-8 p-6 border-t-4 border-purple-500 rounded-lg shadow-md bg-white">
        ...
        <div class="mb-4">
            ...
            <textarea v-model="description" class="w-full p-2 border rounded focus:outline-none focus:border-purple-500"
                placeholder="Enter task description"></textarea>
            <p v-if="descriptionError" class="text-red-500 mt-1">{{ descriptionError }}</p>
        </div>

        <button @click.prevent="addTask" class="w-full bg-purple-500 text-white py-2 px-4 rounded hover:bg-purple-600">
            <div v-if="loading"
